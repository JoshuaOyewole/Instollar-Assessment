...
class MatchService {
  ...
  async updateMatchStatus(matchId, status) {
    try {
      // Validate status
      const validStatuses = ['matched', 'viewed', 'applied'];
      if (!validStatuses.includes(status)) {
        return {
          statusCode: StatusCodes.BAD_REQUEST,
          error: { message: 'Invalid status. Must be matched, viewed, or applied' }
        };
      }

      const updatedMatch = await this.matchRepository.updateStatus(matchId, status);
